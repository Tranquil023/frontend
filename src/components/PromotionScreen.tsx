...
const PromotionScreen: React.FC = () => {
  ...
  const [levels, setLevels] = React.useState([
    { id: 1, title: 'First Level', icon: '🥇', rebate: '₹0.00', rebatePercent: '(15%)', quantity: 0, color: 'yellow-500' },
    { id: 2, title: 'Second Level', icon: '🥈', rebate: '0%', rebatePercent: '', quantity: 0, color: 'yellow-500' },
    { id: 3, title: 'Third Level', icon: '🥉', rebate: '0%', rebatePercent: '', quantity: 0, color: 'yellow-500' },
  ]);
  ...
  return (
    <div className="min-h-screen bg-green-500 pb-24">
      <div className="text-center p-4 pt-8 mb-6">
        <h1 className="text-2xl font-bold text-black">Promotion</h1>
      </div>
        ...
        <div className="bg-yellow-500 rounded-3xl p-6 shadow-xl">
          <div className="grid grid-cols-2 gap-6 text-center">
            <div>
              ...
              <div className="text-black-600">Total People</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-gray-800 mb-1">₹{teamRecharge}</div>
              <div className="text-black-600">Team Recharge</div>
            </div>
          </div>
        </div>

        <div className="bg-yellow-400 rounded-3xl p-4 shadow-xl">
          <div className="flex items-start space-x-3 mb-4">
            <div className="text-2xl mt-1">🎁</div>
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-gray-800 mb-2">Invitation Link</h3>
              <div className="bg-yellow-500 rounded-2xl p-3 mb-3">
                <p className="text-black font-bold text-sm break-all leading-relaxed">{invitationLink}</p>
              </div>
          ...
          <button
            ...
            className={`w-full ${copied ? 'bg-blue-700' : 'bg-green-800'} text-white font-bold py-3 rounded-2xl text-sm transition-all duration-200 flex items-center justify-center space-x-2`}
          >
            {copied ? (
              <>
          ...
          {levels.map(level => (
            <div key={level.id} className="space-y-2">
              <div className="flex items-center justify-between text-black">
                ...
                <button className="flex items-center space-x-1 text-black hover:text-white transition-colors duration-200">
                  <span className="text-sm">View more</span>
                  <Eye className="w-4 h-4" />
                </button>
              </div>
              <div className={`bg-${level.color} rounded-3xl p-6 shadow-xl`}>
                <div className="grid grid-cols-2 gap-6 text-black text-center">
                  <div>
